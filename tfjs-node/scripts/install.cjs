...
const {
  ...
  customTFLibUri,
  customAddon
} = require('./deps-constants.cjs');
const resources = require('./resources.cjs');
const {addonName} = require('./get-addon-name.cjs');
...
async function build() {
  ...
  // Uncomment this or run it manually to rebuild the binding if it's missing.
  // cp.exec(`npm rebuild`, (err) => console.error(err));
  // Install with node-pre-gyp
  cp.exec(`node "${nodePreGyp}" install ${buildOption}`, (err) => {
    if (err) {
    ...
    if (platform === 'win32') {
      ...
      cp.exec('node scripts/deps-stage.cjs symlink ' + modulePath, (error) => {
        if (error) {
